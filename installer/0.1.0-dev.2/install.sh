...
if [[ -L "$lock_base/.install.lock" || -e "$lock_base/.install.lock" ]]; then [[ ! -L "$lock_base/.install.lock" && -f "$lock_base/.install.lock" && $(stat -c '%u' "$lock_base/.install.lock") == $(id -u) ]] || { echo '導入を中止しました: 不正なlockです。' >&2; exit 1; }; fi
